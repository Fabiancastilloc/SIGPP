...
def registrar_gasto(db: Session, gasto_data: ExpenseCreate, estudiante_id: int) -> Expense:
    """
...
def aprobar_rechazar_gasto(
    ...
    NOTA: Ya no se registra quién aprobó porque la columna aprobado_por_id no existe
    """
    ...
    if gasto.estado != EstadoGasto.PENDIENTE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"El gasto ya fue {gasto.estado.value}"
        )
