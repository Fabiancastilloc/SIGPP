  ...
  <div class="dashboard">
    <nav class="navbar">
      ...
      <div class="navbar-menu">
        ...
        <span class="badge badge-finance">{{ authStore.user?.rol }}</span>
        <button @click="handleLogout" class="btn-logout">Cerrar Sesión</button>
      </div>
    </nav>

    <div class="sidebar">
      <router-link to="/finance/dashboard" class="menu-item active">
        <span>📊</span> Dashboard
      </router-link>
      <router-link to="/finance/projects" class="menu-item">
        <span>📁</span> Todos los Proyectos
      </router-link>
      <router-link to="/finance/expenses" class="menu-item">
    ...
    <div class="main-content">
      <div class="container">
        <h1>Panel de Gestión Financiera</h1>
        <p>Administración de presupuesto y aprobación de gastos</p>
        ...
        <div v-else-if="stats" class="stats-grid">
          ...
          <div class="stat-card stat-card-warning">
            ...
            <div class="stat-details">
              <p>💰 Monto Pendiente: ${{ formatNumber(getPendingAmount()) }}</p>
              <p>✅ Aprobados Hoy: {{ getTodayApproved() }}</p>
            </div>
            <router-link to="/finance/expenses" class="btn btn-warning btn-block">
              Revisar Gastos
            </router-link>
          </div>
          ...
          <div class="stat-card stat-card-success">
            ...
            <div class="stat-value">{{ stats.gastos.gastos_aprobados }}</div>
            <div class="stat-details">
              <p>💸 Monto Total: ${{ formatNumber(stats.gastos.monto_total_aprobado) }}</p>
        ...
        <div class="actions-section">
          ...
          <div class="action-cards">
            <router-link to="/finance/expenses" class="action-card">
              ...
              <p>Revisar y aprobar solicitudes de gastos pendientes</p>
            </router-link>

            <router-link to="/finance/budgets" class="action-card">
              <span class="action-icon">💵</span>
              <h3>Asignar Presupuesto</h3>
              <p>Asignar presupuesto a proyectos validados</p>
            </router-link>

            <router-link to="/finance/reports" class="action-card">
              <span class="action-icon">📊</span>
              <h3>Generar Reportes</h3>
              <p>Exportar reportes financieros en CSV</p>
            </router-link>

            <router-link to="/finance/projects" class="action-card">
              <span class="action-icon">📋</span>
              <h3>Ver Proyectos</h3>
...
const getPendingAmount = () => {
  // Placeholder - deberías calcularlo desde el backend
  return 5000000
}

const getTodayApproved = () => {
  // Placeholder - deberías calcularlo desde el backend
  return 12
}
